...
class LogProcessor(GObject.GObject):
    ...
    def process(self, cb):
        ...
        self.warning_list = []
        self.badbox_list = []
        self.file.load_contents_async(None, self.load_cb, cb)


    def load_cb(self, src, res, cb):
        ...
        try:
            ...
            for m in re.finditer(self.warning_re, log):
                ...
                if msg == "Reference":
                    ...
                    detail = "\\cite{" + detail + "}"
                self.warning_list.append((msg, line, detail))

            for m in re.finditer(self.badbox_re, log):
                line   = int(m.group(1))-1
                detail = ""
                self.badbox_list.append(("Overfull box", line, detail))
        finally:
            cb()
